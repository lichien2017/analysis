...
def train_reader(db, taskdata, splitwordpath, word_dict, label_dict):
    """
    ...
    def reader():
        ...
        jieba.load_userdict(splitwordpath)
        datas = db.trains_data_reader(taskdata)
        for idx, (content, flag) in enumerate(datas):
            seg_list = jieba.cut(content, HMM=False)
            doc = ' '.join(seg_list).encode("utf-8")
...
def test_reader(db, taskdata, splitwordpath, word_dict, label_dict):
    """
    Reader interface for training data
    ...
    def reader():
        ...
        jieba.load_userdict(splitwordpath)
        datas = db.tests_data_reader(taskdata)
        for idx, (sampleid, content, flag) in enumerate(datas):
            seg_list = jieba.cut(content, HMM=False)
            doc = ' '.join(seg_list).encode("utf-8")
    ...
    def reader():
        ...
        with open(file_path, "r") as f:
            for doc in f:
                ...
                print("doc is:", doc)
                print("doc_ids is:", doc_ids)
                yield doc_ids, doc

    return reader

def build_word_dict_db(db, taskdata, splitwordpath, save_path, use_col=1, cutoff_fre=1):
    """
    ...
    :type cutoff_fre: int
    """
    print("taskdata is:", taskdata)
    values = defaultdict(int)

    jieba.load_userdict("/mnt/train_files/cutword/dict21080524.txt")
    datas = db.trains_data_reader(taskdata)
    for idx, (content, flag) in enumerate(datas):
        seg_list = jieba.cut(content, HMM=False)
        doc = ' '.join(seg_list).encode("utf-8")
